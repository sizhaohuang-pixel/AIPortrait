...
class AllowCrossDomain
{
    ...
    public function handle(Request $request, Closure $next, ?array $header = []): Response
    {
        ...
        // 如果配置了 * 则允许所有域名
        if (in_array("*", $corsDomain)) {
            $header['Access-Control-Allow-Origin'] = $origin ?: '*';
        } elseif ($origin && !isset($header['Access-Control-Allow-Origin'])) {
            $info = parse_url($origin);
            $corsDomain[] = $request->host(true);

            if (in_array($origin, $corsDomain) || (isset($info['host']) && in_array($info['host'], $corsDomain))) {
                $header['Access-Control-Allow-Origin'] = $origin;
            }
        }
